...
public class CreateDonationRecord extends HttpServlet {
    ...
    protected void processRequest( HttpServletRequest request, HttpServletResponse response )
        ...
        try( PrintWriter out = response.getWriter() ) {
            ...
            out.printf( "<Select name=\"%s\" value=\"\">", DonationRecordLogic.PERSON_ID);
            ...
            out.println( "Test:<br>" );
            out.printf( "<Select name=\"%s\" value=\"\">", DonationRecordLogic.TESTED);
...
//        parameterMap.put("created", new String[]{new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())});
        try {
            DonationRecord donationRecord = dLogic.createEntity( request.getParameterMap() );
            dLogic.add( donationRecord );
        } catch( Exception ex ) {
            log("CreateDonationRecord doPost", ex);
